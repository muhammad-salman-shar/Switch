...
import com.salmanshar.appswitch.model.SubButton
...
class EditButtonActivity : AppCompatActivity() {
    ...
    private fun render() {
        ...
        root.addView(TextView(this).apply { text = "Edit (${config.type}-button)"; textSize = 20f })

        preview = TextView(this).apply {
            text = "A"; setTextColor(Color.WHITE); gravity = Gravity.CENTER
            ...
            background = GradientDrawable().apply {
                ...
                setColor(config.subButtons.getOrNull(0)?.color ?: 0xFF00C853.toInt())
            }
        }
        ...
        config.subButtons.forEachIndexed { i, sub ->
            ...
            nameRow.addView(TextView(this).apply {
                text = "Name (max 2): "; setTextColor(Color.WHITE)
            })
            nameRow.addView(EditText(this).apply {
                filters = arrayOf(InputFilter.LengthFilter(2))
                setText(sub.name)
                hint = "ex: dk"
                addTextChangedListener(object : android.text.TextWatcher {
                    override fun afterTextChanged(s: android.text.Editable?) {
                        sub.name = s?.toString()?.take(2) ?: ""
                    }
    ...
    private fun updatePreview() {
        ...
        preview.layoutParams = preview.layoutParams.apply {
            width = dp.toInt(); height = dp.toInt()
        }
        preview.setTextSize(TypedValue.COMPLEX_UNIT_PX, dp * 0.4f)
        preview.requestLayout()
        val sub = config.subButtons.getOrNull(0)
        preview.background = GradientDrawable().apply {
            shape = GradientDrawable.OVAL
            setColor(sub?.color ?: 0xFF00C853.toInt())
        }
    }
